...
public class TweetToLexiconFeatureVectorTest extends AbstractFilterTest {

    public TweetToLexiconFeatureVectorTest(String name) {
        super(name);
    }

    /** Creates a default TweetToLexiconFeatureVector filter */
	...
	if ((new File(".." + File.separator + "AffectiveTweets" + File.separator + "build_package.xml")).exists()) {
	    ...
	    f = new TweetToLexiconFeatureVector();
	    weka.core.WekaPackageManager.PACKAGES_DIR = backup;
	} else {
	    f = new TweetToLexiconFeatureVector(); // Hope that the package is installed.
	}
	return f;
    }
