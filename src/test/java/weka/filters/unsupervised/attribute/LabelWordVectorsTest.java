...
public class LabelWordVectorsTest extends AbstractFilterTest {
    ...
    public LabelWordVectorsTest(String name) {
        super(name);
    }

    /** Creates a default LabelWordVectors filter */
	...
	if ((new File(".." + File.separator + "AffectiveTweets" + File.separator + "build_package.xml")).exists()) {
	    ...
	    f = new LabelWordVectors();
	    weka.core.WekaPackageManager.PACKAGES_DIR = backup;
	} else {
	    f = new LabelWordVectors(); // Hope that the package is installed.
	}
	return f;
    }

    /**
     * LabelWordVectors is not suitable for use in a FilteredClassifier, so this just creates a dummy
