...
public class TweetToInputLexiconFeatureVectorTest extends AbstractFilterTest {

	public TweetToInputLexiconFeatureVectorTest(String name) {
		super(name);
	}

	/** Creates a default TweetToInputLexiconFeatureVector filter */
	public Filter getFilter() {
		...
		if ((new File(".." + File.separator + "AffectiveTweets" + File.separator + "build_package.xml")).exists()) {
			...
			f = new TweetToInputLexiconFeatureVector();
			weka.core.WekaPackageManager.PACKAGES_DIR = backup;
		} else {
			f = new TweetToInputLexiconFeatureVector(); // Hope that the package is installed.
		}
		return f;
	}
	...
	public static Test suite() {
		return new TestSuite(TweetToInputLexiconFeatureVectorTest.class);
	}
